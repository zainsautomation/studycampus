...
import { Plus, Search, HelpCircle, Sparkles, Clock, CheckCircle2, Loader2 } from "lucide-react";
...
export default function QandA() {
  ...
  const { data, isLoading, fetchNextPage, hasNextPage, isFetchingNextPage } = useInfiniteQuery({
    ...
    queryFn: async ({ pageParam = 0 }) => {
      let query = supabase
        .from('questions')
        .select(`*, subjects:subject_id(name, color), answers(count)`)
  ...
  const questions = data?.pages.flatMap(p => p) ?? [];
  ...
  return (
    <MainLayout>
      <div className="container px-4 py-6 md:py-8 space-y-5">
        {/* Header */}
        <motion.div className="flex flex-wrap items-start sm:items-center justify-between gap-3" initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.15 }}>
          <ScrollArea className="w-full whitespace-nowrap">
            <div className="flex gap-2 pb-2">
              <Badge variant={statusFilter === "all" ? "default" : "outline"} className="cursor-pointer shrink-0 px-3 py-1.5" onClick={() => setStatusFilter("all")}>
                <Sparkles className="h-3 w-3 mr-1.5" />All
              </Badge>
              <Badge variant={statusFilter === "open" ? "default" : "outline"} className="cursor-pointer shrink-0 px-3 py-1.5" onClick={() => setStatusFilter("open")}>
                <Clock className="h-3 w-3 mr-1.5" />Open
              </Badge>
              <Badge variant={statusFilter === "resolved" ? "default" : "outline"} className="cursor-pointer shrink-0 px-3 py-1.5" onClick={() => setStatusFilter("resolved")}>
                <CheckCircle2 className="h-3 w-3 mr-1.5" />Resolved
              </Badge>
              <div className="w-px bg-border shrink-0 mx-1" />
              <Badge variant={subjectFilter === "all" ? "secondary" : "outline"} className="cursor-pointer shrink-0 px-3 py-1.5" onClick={() => setSubjectFilter("all")}>
                All Subjects
              </Badge>
              {subjects?.map((subject) => (
                <Badge
                  ...
                  style={subjectFilter === subject.id ? { backgroundColor: `${subject.color}20`, color: subject.color, borderColor: subject.color } : undefined}
        ...
        {isLoading ? (
          ...
          <motion.div className="text-center py-16 px-4" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.2 }}>
            ...
            <p className="text-muted-foreground mb-6 max-w-sm mx-auto">Be the first to spark a discussion!</p>
          ...
          <div className="space-y-3">
            {questions.map((question, index) => (
              <QuestionCard key={question.id} question={question} onClick={() => navigate(`/qa/${question.id}`)} index={index} />
            ))}
